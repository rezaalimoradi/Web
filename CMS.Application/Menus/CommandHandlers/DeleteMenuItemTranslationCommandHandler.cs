...
{
    public class DeleteMenuItemTranslationCommandHandler : IAppRequestHandler<DeleteMenuItemTranslationCommand, ResultModel<long>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly ITenantContext _tenantContext;

        public DeleteMenuItemTranslationCommandHandler(IUnitOfWork unitOfWork, ITenantContext tenantContext)
        {
            _unitOfWork = unitOfWork;
            _tenantContext = tenantContext;
        }

        public async Task<ResultModel<long>> Handle(DeleteMenuItemTranslationCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                menuitem.RemoveTranslation(menuitem.Translations.FirstOrDefault()!);

                var result = await _unitOfWork.SaveChangesAsync(cancellationToken);
                if (result > 0)
                {
                    return ResultModel<long>.Success(menuitem.Translations.FirstOrDefault().Id);
                }
